...
namespace CNC_Assist
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            // При запуске проверим наличие утилит
            string pathapp = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);

            if (File.Exists(pathapp + @"\ToolsImporterTextImage.exe"))
            {
                toolsImporterTextImageToolStripMenuItem.Enabled = true;
            }


            if (File.Exists(pathapp + @"\ToolsImporterVectors.exe"))
            {
                toolsImporterVectorsToolStripMenuItem.Enabled = true;
            }
        ...
        private void CncNewData(object sender)
        {
            ...
            //TODO: необходимо оповестить и другие элементы
            
            // простой вызов не возможен, т.к. процедура эта вызывается не из основного потока
        ...
        private void RefreshElementsForms(bool _reloadPanel = false)
        {
            ...
            {
                bt_ConnDiskonect.Image = Resources.disconnect;
        ...
        private void SendSetting()
        {
            ...
            ControllerPlanetCNC.AddBinaryDataToTask(BinaryData.pack_A0(GlobalSetting.ControllerSetting.AxleX.Acceleration,
                ...
                GlobalSetting.ControllerSetting.AxleX.reversAxle,
                GlobalSetting.ControllerSetting.AxleY.reversAxle,
                GlobalSetting.ControllerSetting.AxleZ.reversAxle,
                GlobalSetting.ControllerSetting.AxleA.reversAxle,
                GlobalSetting.ControllerSetting.AxleX.reversSignal,
                GlobalSetting.ControllerSetting.AxleY.reversSignal,
        ...
        private Constructor.ConstructorForm _consructorform = null;


        private void ConstructorForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            _consructorform = null;
        }

        private void toolsConstructorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (_consructorform == null)
            {
                _consructorform = new Constructor.ConstructorForm();
                _consructorform.FormClosed += new FormClosedEventHandler(ConstructorForm_FormClosed);
                _consructorform.Show();
            }
            else
            {
                _consructorform.Show();
        ...
        private void toolsImporterTextImageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string pathapp = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);

            Process.Start(pathapp + @"\ToolsImporterTextImage.exe");
        }

        private void toolsImporterVectorsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string pathapp = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
